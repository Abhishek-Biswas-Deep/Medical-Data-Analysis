...
Answer-
note_id- It is an integer that works as a unique identifier for the note.
description- It is providing a description of the patient's issue.
medical_specialty- It is the medical specialty relevant to the clinical note.
sample_name- It is a title for the specific clinical sample.
transcription- It contains the full text of the clinical note.
keywords- It is contains keywords associated with the clinical note.
...
unique_unigrams_count <- tokenized.data.unigram %>%
  ...
  summarise(unique_unigrams = n())

print(unique_unigrams_count)

```
Answer- Orthopedic	7682			
Radiology	5935			
Surgery	11977	
...
```{r}
unique_bigrams_count <- tokenized.data %>% 
  group_by(medical_specialty, ngram) %>% 
  summarise(count = n()) %>%
  group_by(medical_specialty) %>%
  summarise(unique_bigrams = n())

print(unique_bigrams_count)
```
Answer-
Orthopedic= 55732
Radiology= 28297			
...
```
Answer- So, there are 355 unique words in Orthopedic, 273 unique words in Radiology and 1088 unique words in Surgery.
...
Answer- 
I think a general purpose lemmatizer will not work well. This is because, it will miss many terms in medical data and there will be many other specialized terms that might be never used. So, a general purpose lemmatizer will not be able to look into that.
...
Answer-
The plot tells me that Surgery and Orthopedic share many terms more than radiology and Surgery. There happen to be matching frequencies between Surgery and Orthopedic. The points for Radiology and Surgery are dispersed so, it might not be similar in frequencies.
...
Answer-
There are many but the ones that stand out are Orthopedic of n = 139 which indicates its significance within this specialty, in Radiology of n = 50 and in Surgery of n = 174. They stand out because they have higher TF-IDF scores which indicates that they are both frequent within their respective specialties and rare in other specialties. 
...
dplyr::filter(stringr::str_detect(transcription, 'myocardial perfusion')) %>% dplyr::slice(1)
```
## Topic Modelling
...
```
Answer- 
Upon expanding the LDA model to six topics, the terms identified in the 3-topic LDA model are still present, but they are now allocated differently across the new topics. This expansion reduces the overlap between specialties, resulting in a clearer distinction. Orthopedic is now primarily represented by topics 1 and 2, Surgery by topics 3 and 4, and Radiology by topics 5 and 6. Each specialty is thus mainly associated with two of the six new sub-topics.
