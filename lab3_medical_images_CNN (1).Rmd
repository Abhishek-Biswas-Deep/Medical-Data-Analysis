...
Answer-
Pneumonia is an infection in the lungs, usually caused by bacteria or viruses, leading to inflammation and accumulation of fluid. It makes it hard to breathe and causes chest pain. 
Being able to identify them can save time for doctors. It is useful to detect pneumonia at an earlier stage and rememdy the issue and it can also help in improving the diagnosis.
...
Answer- 
The filenames indicates pneumonia. But those don't explicitly indicate the presence of pneumonia. This could make predicting pneumonia more challenging because the model must learn to distinguish between normal and pneumonia X-rays based on visual features rather than relying on the filenames.
...
Answer-
The images have varying levels of brightness and contrast. Same amount of brightness and contrast should be normalized for consistent analysis. The images are not consistently oriented which can affect model training, so all the images can have proper and consistent orientation which can be normalised.
...
Answer-
If the dataset was not balanced, assigning higher weights to samples from underrepresented classes can be done. Predictions from multiple models can also be combined to improve overall accuracy. The frequency of sample can be adjusted as well.
...
Answer-
Data augmentation is time consuming. It can increase the time required for training a  model, when the augmentation is performed during training epochs. Data augmentation can sometimes cause overfitting, such as using too many transformations might make the model learning something new and does not align with the data. Sometimes, augmented datasets can be larger in size, so it might require more storage space.
...
Answer-
ImageNet is a widely recognized dataset in the field of computer vision. It consists of over 14 million images annotated with more than 20,000 categories. The ImageNet Challenge, held in 2012, was a competition designed to evaluate algorithms for object detection and image classification on a large scale.
This dataset enables researchers to develop models that can generalize across a wide variety of visual concepts, enhancing their performance on real-world visual recognition tasks. So, this is how it can be useful.
...
Answer-
Using the MobileNet architecture in this practical assignment can help to improve results due to its efficient design and performance characteristics. It employs depthwise separable convolutions to reduce the number of parameters and computational requirements while maintaining accuracy, making it cost-efficient. Therefore, it is useful to solve practical assignments.

**9_** How many parameters does this network have? How does this compare to better performing networks available in torch?
Answer-
This network has 3.5 million parameters and 10 million parameters for the final layer. It also has 0.56 Million mult-adds. Despite its smaller parameter count, it is capable of achieving competitive performance on various computer vision tasks due to its efficient design and the use of depthwise separable convolutions. So, it is a great feature which is capable of outweighing other models.
