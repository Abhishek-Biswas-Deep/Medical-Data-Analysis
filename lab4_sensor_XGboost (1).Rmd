...
Answer-
14980/117 = 128
Approximately 128 samples were taken in a second.

**1** How many EEG electrodes/sensors were used?
Answer-
14 electrodes/sensors were used.
...
Answer-
There is a spike in the electric potential whenever eyes are opened.
...
Answer-
Even though there are temporal correlation between eye open/close states, but it is not seen in the above plot. This is because the intensities fall from all the 14 EEG channels overlap and the time duration is also not consistent.
...
Answer-
AF3 electrode: Here the mean intensity is slightly higher when the eyes are closed. 
The standard deviation is slightly lower when the eyes are closed, indicating less variability.

F7 electrode: Here the mean intensity is lower when the eyes are closed.
The standard deviation is lower when the eyes are closed, indicating less variability.

F3 Electrode: Here the mean intensity is very similar regardless of eye status.
The standard deviation is lower when the eyes are closed, indicating less variability.

FC5 Electrode: Here the mean intensity is very similar regardless of eye status.
The standard deviation is slightly higher when the eyes are closed, indicating slightly more variability.

T7 Electrode: Here the mean intensity is slightly higher when the eyes are closed.
The standard deviation is slightly higher when the eyes are closed, indicating slightly more variability.

In conclusion, AF3 Electrode shows consistently higher mean intensities when eyes are closed, along with slightly less variability.
F7 Electrode shows lower mean intensities and less variability when eyes are closed.
F3 Electrode and FC5 Electrode have similar mean intensities, but F3 shows less variability when eyes are closed while FC5 shows slightly more variability.
T7 Electrode shows higher mean intensities and slightly more variability when eyes are closed.
Thus, AF3 and T7 electrodes seem to be more intense when the eyes are closed, while F7 is more intense when the eyes are open. The variability is generally lower when the eyes are closed for most electrodes except FC5 and T7, where it is slightly higher.
...
Answer-
Stationarity means the behaviour of time series data such as EEGs and ECGs to exhibit a constant statistical characteristics over time. For instance, if the statistical properties, such as mean, variance, and autocorrelation structure remain constant over time, it would be considered stationary.
...
Answer- 
We are interested because it ensures that the assumptions of time series models hold true. Simplifies the analysis and interpretation of data. Provides a stable basis for accurate forecasting.
The results tell us that most of the EEG electrodes time series data are stationary. So it allows for the use of standard time series modeling techniques, which can provide reliable and interpretable results for further analysis and forecasting.
...
Answer-
Autocorrelation:
Electrode 'FC5' shows strong negative autocorrelation. Because, at time lag, t=0, the value of ACF is about 0.50. 'FC6' also shows strong positive autocorrelation. Because, at time lag, t=0, the value of ACF is about 0.40.
Cross-correlation:
There is a negative cross-correlation between electrodes 'F7' and 'FC5' and there is a positive cross-correlation between electrodes 'FC6' and 'T8'. There is also positive cross-correlation between electrodes 'F4' and 'FC6'.
...
Answer-
When the eye state is closed, channels 1, 9 and 13 have a constant power spectral density of about +40dB but the other channels are from 0 to -20dB. When the eye state is open, the channels 6 and 14 have a constant power spectral density of about +40dB and +60dB but the other channels are from 0 to +20dB. When eye state is opened, large number of electrice spikes are observed in comparison to eye closed state and this happens for mostly every channel.
...
Answer-
The eye opening state shows that ICA has successfully isolated the eye activity as an independent component. This is because the majority of the activity in the plot is in the V1 channel. But I do not see the V2 and V3 channel anywhere.
...
model2 <- randomForest(x = eeg_train_matrix, 
                      y = eeg_train_labels, 
                      ntree = 100,
                      maxnodes = 16)
...
```{r test}
predictions_xgb <- predict(model, newdata = eeg_validate_matrix)
predictions_rf <- predict(model2, newdata = eeg_validate_matrix)

accuracy_xgb <- mean(predictions_xgb == eeg_validate_labels)
accuracy_rf <- mean(predictions_rf == eeg_validate_labels)

cat("XGBoost Model Accuracy on Validation Set:", accuracy_xgb, "\n")
cat("Random Forest Model Accuracy on Validation Set:", accuracy_rf, "\n")

eeg_test_matrix <- as.matrix(dplyr::select(eeg_test, -eyeDetection, -ds))
eeg_test_labels <- as.numeric(eeg_test$eyeDetection) - 1

predictions_test <- predict(model2, newdata = eeg_test_matrix)
accuracy_test <- mean(predictions_test == eeg_test_labels)

cat("Random Forest Model Accuracy on Test Set:", accuracy_test, "\n")
```
...
Answer-
Bayesian approach: The probability Bayes theorem is applied in this method. We can determine the conditional probability by choosing the class with the biggest posterior probability and using the observed facts that we already know.

State-space approach (Hidden Markov Models): A sequence of hidden states could be used to represent a time series of data, with the transition probabilities from the hidden states determining the value of the data, or the next state.
...
Answer-
Bayesian approach: BLR
State-space approach (HMM): HiddenMarkov
...
Answer-
Overall, it was a fantastic course. I got to learn a lot about medical science even though I am a CS student. Thank you. 

One suggestion is practical 3 caused a lot of trouble for me. So, if you look into that and fix some issues then it would be beneficial for future students, if they use MacBook of course!
